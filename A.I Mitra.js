...
const BarSlider = () => {
  console.log("c"); // Debugging statement to check function call
  chatsec.style.marginLeft = "-290px"; // Slide chat section left
  setTimeout(() => { utsec.style.gap = "8rem"; }, 2000); // Adjust gap in the wrapper after 2 seconds
  setTimeout(() => { slideBackBtn.style.display = "flex"; }, 2000); // Show the back arrow button
};

// Function to slide back the chat section when the arrow button is clicked
const ArrowSlider = () => {
  chatsec.style.marginLeft = "0"; // Reset chat section to its original position
  utsec.style.gap = "0rem"; // Remove gap in the wrapper
  setTimeout(() => { slideBackBtn.style.display = "none"; }, 5000); // Hide the back arrow after 5 seconds
};
